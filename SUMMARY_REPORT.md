...
**MNIST** (60,000 training images, 28×28 grayscale). Chosen for fast iteration (~45–54s per diffusion run on Apple MPS) and well-understood visual failure modes.

### Controlled Experiment Design

**Knob 1:** T ∈ {100, 200, 400} — diffusion timesteps
**Knob 2:** target ∈ {eps, x0} — prediction parameterization
...
|---|---------------|---------------|-------------|-------------|
| 100 | 0.1042 | **0.0258** | 44.5 s | 45.6 s |
| 200 | 0.0886 | 0.0503 | 46.5 s | 47.4 s |
| 400 | **0.0594** | 0.1114 | 52.9 s | 53.5 s |

- **eps loss decreases with larger T** (0.104 → 0.059). With more timesteps, each individual noise step is smaller, making per-step noise prediction a smoother regression target and easier to learn.
- **x0 loss increases with larger T** (0.026 → 0.111). With more timesteps, the model must reconstruct clean images from inputs at higher noise levels (larger t values), where x_t ≈ pure Gaussian noise — a fundamentally harder inverse problem.
...
Training time scales approximately linearly with T: ~45s (T=100) → ~53s (T=400), a ~19% increase. The eps vs x0 choice does not significantly affect runtime (< 1s difference per run). Total wall-clock time for all 6 diffusion runs: ~4.8 minutes.

### 3.4 Transformer Baseline

- Train loss: 3.443 → 0.053 in 600 steps (sharp descent, 13 eval points).
- Val loss: 3.444 → 0.054 (nearly identical to train throughout, gap < 0.01).
- Sample output: garbled fragments ("Transformers replace rrececurrrapne with attention. Self-aturenrelati-antSenlfiti...").
- Runtime: 11.1s on Apple MPS.

The near-identical train/val loss reveals pure memorisation on the tiny ~330-character corpus. The model memorises token co-occurrences but cannot compose novel sequences — temperature-based sampling exposes the lack of generalisation. The training curve shows smooth exponential decay with no train/val divergence.
...
**Evidence:** x0 loss at T=400 (0.111) is *worse* than at T=100 (0.026). This trend inverts relative to eps.
...
**Evidence:** Train ≈ val loss throughout 600 steps (gap < 0.01); sample output is garbled mixed fragments even at loss=0.053.
...
- The Transformer experiment serves as a pipeline check, not a deep behavioural study (because of the small corpus).
...
3. **Low per-step loss does not guarantee good samples.** T=100 x0 achieves the lowest raw loss (0.026) but does not produce the best visual samples, which accumulated error over too few denoising steps still degrades quality.

4. **Runtime scales linearly with T** (~19% increase from T=100 to T=400) without proportional quality gains at 1 epoch.
