...
def get_dataset(name: str, data_dir: str, download: bool) -> Tuple[torch.utils.data.Dataset, int]:
    ...
    if name in ["mnist"]:
        ds = datasets.MNIST(root=str(base), train=True, download=download, transform=tfm)
        return ds, 1
    if name in ["fashion", "fashionmnist", "fmnist"]:
        ds = datasets.FashionMNIST(root=str(base), train=True, download=download, transform=tfm)
        return ds, 1
...
def make_run_paths(run_name: str, out_dir: str = "./untrack/outputs/final/gan") -> RunPaths:
...
def run_one(args: argparse.Namespace, overrides: Dict[str, str]) -> Path:
    ...
    paths = make_run_paths(run_name, out_dir=args.out_dir)
