...
def _capture_losses_from_stdout(text: str) -> List[Tuple[int, float]]:
    """Parse 'epoch XX step YYYYYY | loss=Z.ZZZZ' lines from captured stdout."""
    import re
    pattern = re.compile(r"step\s+(\d+)\s*\|\s*loss=([\d.]+)")
    out = []
...
def run_single(args: argparse.Namespace, overrides: Dict[str, str]) -> Path:
    ...
    # Echo captured output and save train_log.csv
    captured_text = captured.getvalue()
    print(captured_text, end="")
    losses = _capture_losses_from_stdout(captured_text)
    if losses:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=str, default="./final/untrack/outputs/diffusion")
